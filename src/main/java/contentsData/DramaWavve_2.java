	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int pageCount=1; pageCount<=5; pageCount++) {
				...
				URL firstURL = new URL("https://api.themoviedb.org/3/discover/tv?api_key=4b5fa5612cda62f4af304556025d6fc5&language=ko&sort_by=popularity.desc&page="+pageCount+"&include_null_first_air_dates=false&with_watch_providers=356&watch_region=KR&with_watch_monetization_types=flatrate&with_type=2");
				BufferedReader bf1;
				...
				String sql = "MERGE INTO drama USING DUAL ON (dr_id = ?) WHEN MATCHED THEN UPDATE SET dr_ottWV = 'Y' WHEN NOT MATCHED THEN INSERT (dr_id,dr_title,dr_genre,dr_first_air_date,dr_vote_average,dr_ottNF,dr_ottWV,dr_ottDZ,dr_ottWC,dr_like,dr_poster_path,dr_overview) VALUES (?,?,?,?,?,DEFAULT,'Y',DEFAULT,DEFAULT,DEFAULT,?,?)";
