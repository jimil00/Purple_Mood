	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int pageCount=1; pageCount<=10; pageCount++) {
				...
				String sql = "MERGE INTO movie USING DUAL ON (mv_id = ?) WHEN MATCHED THEN UPDATE SET mv_ottWV = 'Y' WHEN NOT MATCHED THEN INSERT (mv_id,mv_title,mv_genre,mv_release_date,mv_vote_average,mv_runtime,mv_ottNF,mv_ottWV,mv_ottDZ,mv_ottWC,mv_like,mv_poster_path,mv_overview) VALUES (?,?,?,?,?,?,DEFAULT,'Y',DEFAULT,DEFAULT,DEFAULT,?,?)";
