				...
				<c:when test="${loginNickname == dto.b_writer}">
					...
					<tr>
						...
						<a href="/beforeUpdateBoardContents.board?b_seq=${dto.b_seq }"><input
							...
							href="/boardList.board"><input type="button" id="toList"
								name="toList" value="목록으로"></a>
					</tr>
				</c:when>
				<c:otherwise>
					<tr>
						...
						<tr>
							...
							<c:choose>
								<c:when test="${loginNickname == comment.bcm_writer}">
									...
									<a href="/deleteBoardComment.boardcomment?bcm_seq=${comment.bcm_seq }&b_seq=${dto.b_seq}"><button type="button" class="deleteBoardComment" name="deleteBoardComment">댓글삭제</button></a>
									</td>
								</c:when>
								<c:otherwise>
									<td><a href="insertBoardCommentComplain.boardcommentcomplain">신고</a></td>
								</c:otherwise>
							</c:choose>
						</tr>
