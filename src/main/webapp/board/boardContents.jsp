		...
		<table border="1" width="500" align="center">
			<tr>
				...
				<td width="70%" colspan="2">
					<input type="text" id="b_title" name="b_title"
						value="${dto.b_title }" size="129" readonly>
				</td>
			</tr>
			<tr>
			...
			<c:choose>
				<c:when test="${loginNickname == dto.b_writer}">
					...
					<tr>
						<td class="btns" colspan="3" align="right">
							<input type="button" class="btn" id="updateBoardContents"
								name="updateBoardContents" value="수정하기">
							&nbsp
							<a href="/deleteBoardContents.board?b_seq=${dto.b_seq }">
								<input type="button" class="btn" id="deleteBoardContents"
									name="deleteBoardContents" value="삭제하기">
							</a>
							&nbsp
							<a href="/boardList.board">
								<input type="button" id="toList" name="toList" value="목록으로">
							</a>
					</tr>
				</c:when>
				<c:otherwise>
					<tr>
						...
						<td class="btns" colspan="3" align="right">
							<a href="/boardList.board?cpage=${boardPage }">
								<input type="button" id="back" name="back" value="목록으로">
							</a>
					</tr>
				</c:otherwise>
			</c:choose>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="comment" items="${list}">
					...
					<tr>
						<td>
							<input type="text" class="bcm_content" name="bcm_content"
								size="105" value="${comment.bcm_content }"
								bcm_seq="${comment.bcm_seq }" readonly>
						</td>
						<c:choose>
							<c:when test="${loginNickname == comment.bcm_writer}">
								<td class="cbtns" align="center">
									<input type="button" class="updateBoardComment"
