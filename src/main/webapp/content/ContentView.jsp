          ...
          <div class="col-sm-9">
            <div class="row">
              ...
              <div class="info col-12">
                <div><span>장르:</span> <span>${mv_detail.getMv_genre()}</span></div>
                <div><span>출시년도:</span> <span>${mv_detail.getMv_release_date()}</span></div> 
                 <div><span>상영시간:</span> <span>${mv_detail.getMv_runtime()}</span></div> 
                <div><span>평점:</span> <span>${mv_detail.getMv_vote_average()}</span></div>
              </div>
              <div class="overview col-12">
                <br>
                <p>줄거리</p> 
                 <p>${mv_detail.getMv_overview()}</p> </div>
              </div>
            </div>
        </div>
			...
			<!-- 드라마 정보 출력 -->
              ...
              <div class="col-sm-9">
                <div class="row">
                  ...
                  <div class="info col-12">
                    <div><span>장르:</span> <span>${dr_detail.getDr_genre()}</span></div>
                <div><span>방영날짜:</span> <span>${dr_detail.getDr_first_air_date()}</span></div> 
                <div><span>평점:</span> <span>${dr_detail.getDr_vote_average()}</span></div>
                  </div>
                  <div class="overview col-12">
                    <br>
                    <p>줄거리</p> 
                    <p>${dr_detail.getDr_overview()}</p> </div>
                  </div>
           </c:otherwise>
            </c:choose>
